...
return"";}
function changeTheme(name){var element=document.getElementsByTagName("body")[0];if(element){if(!element.classList.contains(name)){element.classList.add(name)
setCookie('theme',name,3);}}}
function removeTheme(name){var element=document.getElementsByTagName("body")[0];if(element){element.classList.remove(name)
setCookie("theme","",3);}}
function switchTheme(){var theme=getCookie('theme');var index=themeList.indexOf(theme)+1;if(index>0){removeTheme(theme);if(themeList.length===1||index===themeList.length){return}}
return changeTheme(themeList[index]);}
var themeList=["theme-tree"];var theme=getCookie('theme');if(themeList.indexOf(theme)>-1){changeTheme(theme);}
...
dsq.type='text/javascript';dsq.src='https://utteranc.es/client.js';dsq.setAttribute('repo','honmaple/honmaple.github.io');dsq.setAttribute('issue-term',thr.getAttribute('issue-term'));dsq.setAttribute('label','comment');dsq.setAttribute('theme','github-light');dsq.setAttribute('crossorigin','anonymous');thr.appendChild(dsq);$(".entry-comment").parent().fadeToggle(800);});$(".entry-switch-theme").click(function(){switchTheme();});$(".navbar-logo").error(function(){var content='<span class="navbar-title">'+$(".navbar-logo").attr("alt")+'</span>';$(".navbar-logo").parent().html(content);});$('#entry-decrypt-password').keyup(function(e){if(e.keyCode==13){decrypt($(this).next());}});$(".entry-decrypt").click(function(e){decrypt($(this));});});function ISPC(){var userAgentInfo=navigator.userAgent;var agents=["Android","iPhone","SymbianOS","Windows Phone","iPad","iPod"];var flag=true;for(var v=0;v<agents.length;v++){if(userAgentInfo.indexOf(agents[v])>-1){flag=false;break;}}
return flag;}
function Cover(){if(!String.prototype.format){String.prototype.format=function(){var args=arguments;return this.replace(/{(\d+)}/g,function(match,number){return typeof args[number]!='undefined'?args[number]:match;});};};if($(window).width()>768){$.ajax({type:"GET",url:"https://poem.honmaple.com/api/poem/random",dataType:"json",success:function(response){var title='<h3>{0}</h3>'.format(response.data.title);var author='<p>{0}</p>'.format(response.data.author);var paragraphs='<div>{0}</div>'.format(response.data.paragraphs.map(function(item){return"<p>{0}</p>".format(item);}).join(""));$(".entry-cover > .entry-cover-right").fadeOut(500,function(){$(this).html(title+author+paragraphs).fadeIn(500);});}});};function scrollUp(){if($(window).scrollTop()<$(window).height()&&$(".entry-cover").length&&!$(".entry-cover").is(":hidden")){$("html, body").animate({scrollTop:0},600);}}
